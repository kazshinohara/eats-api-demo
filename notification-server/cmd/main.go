...
import (
	...
	"context"
	"encoding/json"
	pb "github.com/kazshinohara/pb/eats-api-demo"
	"google.golang.org/grpc"
	"google.golang.org/grpc/codes"
	"google.golang.org/grpc/status"
	"log"
	"net"
	"notification.com/server/model"
	"os"
	"sync"
	"time"
)
...
type notificationServer struct {
	stream      pb.UnimplementedNotificationServer
	subscribers sync.Map
}

type subsub struct {
	stream   pb.Notification_GetNotificationServer
	finished chan<- bool
}
...
func (s *notificationServer) GetNotification(request *pb.NotificationRequest, stream pb.Notification_GetNotificationServer) error {
	log.Printf("Reveived get notification-server request")
	...
	for {
		...
		s.subscribers.Range(func(k, v interface{}) bool {
			...
			for _, e := range msgList {
				if err := sub.stream.Send(&pb.NotificationReply{
